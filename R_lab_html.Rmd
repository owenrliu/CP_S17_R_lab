...
all_threats <- raster("D:/TA files/CP2017_Owen/R lab development/Threats_data/full_modelnv.tif")
```
...
all_spp <- raster("D:/TA files/CP2017_Owen/R lab development/Species_data/ca_curr_sp_rich.tif")
all_spp
...
```{r example fishing, echo=F}
fish_files <- list.files("D:/TA files/CP2017_Owen/R lab development/Threats_data",full.names = T)
fish_files <- fish_files[c(10:12,23,24,28)]
...
fish_stack <- reclassify(fish_stack,rcl=c(-Inf,0,NA),progress="text")
names(fish_stack) <- c("dem_d","dem_hb","dem_lb","pel_hb","pel_lb","rec")

# maximum of any fishing pressure in each cell
fish_max <- overlay(fish_stack,fun=max)
...
cols = rev(colorRampPalette(brewer.pal(11, 'Spectral'))(5)) 

library(tmap)
bb<-c(-125,-110,22,40)
data(World)
fishing <- tm_shape(fish_max,bbox=bb)+
